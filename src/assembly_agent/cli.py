...
def _resolve_agent_id(args, key: str) -> Optional[str]:
    ...
    for agent in list_agents(assemblyai_api_key=key):
...
def cmd_phone_import(args):
    ...
    key = _key()
    import_number(args.number, args.trunk, assemblyai_api_key=key)
    agent_id = _resolve_agent_id(args, key)
    if agent_id:
        assign_number(args.number, agent_id, assemblyai_api_key=key)
    print("ok")
...
def cmd_phone_unassign(args):
    ...
    unassign_number(args.number, assemblyai_api_key=_key())
    print("ok")
...
def cmd_phone_list(args):
    ...
    _emit(list_numbers(assemblyai_api_key=_key(), limit=args.limit))


def cmd_phone_release(args):
    from .phones import release_number

    release_number(args.number, assemblyai_api_key=_key())
    print("ok")
...
def cmd_agents_list(args):
    ...
    _emit(list_agents(assemblyai_api_key=_key()))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    buy = pcmds.add_parser("buy", help="buy a number and assign it to an agent")
    ...
    imp = pcmds.add_parser("import", help="import a number you own (BYO trunk)")
    ...
    assign = pcmds.add_parser("assign", help="assign/re-assign an owned number")
    assign.add_argument("number")
    _add_agent_opts(assign)
    assign.set_defaults(func=cmd_phone_assign)

    unassign = pcmds.add_parser("unassign", help="un-assign a number")
    unassign.add_argument("number")
    unassign.set_defaults(func=cmd_phone_unassign)

    plist = pcmds.add_parser("list", help="list owned numbers")
    plist.add_argument("--limit", type=int, default=20)
    plist.set_defaults(func=cmd_phone_list)

    release = pcmds.add_parser("release", help="release a number back to the provider")
    ...
    alist = acmds.add_parser("list", help="list agents (name + id)")
